...
- 系统进入休眠时自动停止录制并尝试保存；唤醒后保持停止，按快捷键可开始新录像。
- 保存失败时保留原始恢复文件和剩余临时文件。
...
- 合盖触发系统休眠时会停止录制；外接显示器等使电脑合盖后继续运行的情形，不会触发这项保护。
- 系统可能在收尾完成前进入休眠，保存会在唤醒后继续；不会自动开启新的录制。仍需在真实设备上验证合盖行为。
- 保存失败时会尝试将原始内容保留为 `~/Movies/QuietRecorder/Recovery_<UUID>.mov`，并保留剩余隐藏 partial 文件；未完成封装的文件不保证可以播放。恢复副本创建失败时保留原文件并记录日志。
